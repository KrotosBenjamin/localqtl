...
def calculate_association(genotype_df, phenotype_s, covariates_df=None,
    ...
    if haplotype_df is not None:
        ...
        haplotypes_t = torch.tensor(haplotype_df.values, dtype=torch.float32).to(device)

    dof = phenotype_s.shape[0] - 2
    residualizer = None
    if covariates_df is not None:
        assert phenotype_s.index.equals(covariates_df.index)
        cov_t = torch.tensor(covariates_df.values, dtype=torch.float32).to(device)
...
def map_nominal(genotype_df, variant_df, phenotype_df, phenotype_pos_df, prefix,
    ...
    if haplotype_df is not None:
        logger.write(f"  * {haplotype_df.shape[0]} haplotype/ancestry tracks")
    ...
    if interaction_df is not None:
        ...
        if maf_threshold_interaction > 0 and ni == 1:
            ...
            mask_s[interaction_df[interaction_df.columns[0]]
                   .sort_values(kind='mergesort')
                   .index[:interaction_df.shape[0] // 2]] = False
            interaction_mask_t = torch.BoolTensor(mask_s.values).to(device)
        else:
        ...
        if ni == 1:
            ...
            col_order += [k.replace('i', f"i{i+1}") for i in range(0,ni)
                          for k in ['pval_i', 'b_i', 'b_i_se', 'pval_gi', 'b_gi', 'b_gi_se']]

        # Rename interaction outputs to use column names
        var_dict = {}
        for i, v in enumerate(interaction_df.columns, 1):
            for c in ['pval_i', 'b_i', 'b_i_se']:
                var_dict[c.replace("_i", f"_i{i}")] = c.replace("_i", f"_{v}")
            for c in ['pval_gi', 'b_gi', 'b_gi_se']:
                var_dict[c.replace("_gi", f"_gi{i}")] = c.replace("_gi", f"_g-{v}")
    ...
    # Prepare InputGenerator with haplotypes
    ...
    k = 0 # initialize counter for enumerate offset
    ...
    for chrom in igc.chrs:
        ...
        start = 0
        if group_s is None:
            for k, (phenotype, genotypes, g_idx, haplotypes, _, phenotype_id) in \
                    enumerate(igc.generate_data(chrom=chrom, verbose=verbose), k+1):
                ...
                if haplotypes is not None:
                    haplotypes_t = torch.tensor(haplotypes, dtype=torch.float).to(device)
                else:
                    haplotypes_t = None

                variant_ids = variant_df.index[g_idx[0]:g_idx[-1]+1]
                start_distance = np.int32(variant_df['pos'].values[g_idx[0]:g_idx[-1]+1] -
                                          igc.phenotype_start[phenotype_id])
                if 'pos' not in phenotype_pos_df:
                    end_distance = np.int32(variant_df['pos'].values[g_idx[0]:g_idx[-1]+1] -
                ...
                if paired_covariate_df is None or phenotype_id not in paired_covariate_df:
                    ...
                    pcov_t = torch.tensor(np.c_[covariates_df, paired_covariate_df[phenotype_id]],
                                          dtype=torch.float32).to(device)
                ...
                if interaction_df is None:
                    res = calculate_cis_nominal(genotypes_t, phenotype_t,
                                                haplotypes_t=haplotypes_t,
                                                residualizer=iresidualizer)
                    tstat, slope, slope_se, af, ma_samples, ma_count = [i.cpu().numpy() for i in res]
                    n = len(variant_ids)
                else:
                    genotypes_t, mask_t = filter_maf_interaction(
                        genotypes_t, interaction_mask_t=interaction_mask_t,
                        maf_threshold_interaction=maf_threshold_interaction
                    )
                    if genotypes_t.shape[0] > 0:
                        ...
                        res = calculate_interaction_nominal(
                            genotypes_t, phenotype_t.unsqueeze(0),
                            interaction_t, haplotypes_t=haplotypes_t,
                        ...
                        # top association (interactions tested)
                        ix = np.nanargmax(np.abs(tstat[:,1+ni:]).max(1))
                        # index order: 0, 1, 1+ni, 2, 2+ni, 3, 3+ni, ...
                        order = [0] + [i if j % 2 == 0 else i+ni
                                       for i in range(1,ni+1) for j in range(2)]
                        ...
                        if run_eigenmt:  # compute eigenMT correction
                            top_s['tests_emt'] = eigenmt.compute_tests(
                                genotypes_t, var_thresh=0.99, variant_window=200
                            )
            ...
            for k, (phenotype, genotypes, g_idx, haplotypes, _, phenotype_id, group_id) in \
                    enumerate(igc.generate_data(chrom=chrom, verbose=verbose), k+1):
                ...
                if genotypes_t.shape[0] > 0:
                    ...
                    for phenotype, phenotype_id in zip(phenotypes[1:], phenotype_ids[1:]):
                        ...
                        if interaction_df is None:
                            res = calculate_cis_nominal(
                                genotypes_t, phenotype_t, residualizer=iresidualizer,
                                haplotypes_t=haplotypes_t
                            )
                            ...
                            res = calculate_interaction_nominal(
                                ...
                                return_sparse=False, variant_ids=variant_ids)
                    ...
                    if interaction_df is not None:
                        ...
                        order = [0] + [i if j % 2 == 0 else i+ni
                                       for i in range(1, ni+1) for j in range(2)]
                        top_s = [chr_res['phenotype_id'][start:start+n][ix],
                                 variant_ids[ix], start_distance[ix]]
                        if 'pos' not in phenotype_pos_df:
                            top_s += [end_distance[ix]]
                        top_s += [af[ix], ma_samples[ix], ma_count[ix]]
                        for i in order:
        ...
        if write_stats:  # write full summary stats for current chromosome
            ...
            if interaction_df is None:
                ...
                chr_res_df.loc[m, 'pval_nominal'] = get_t_pval(
                    chr_res_df.loc[m, 'pval_nominal'], idof, log=logp
                )
            else:
                if ni == 1:
                    ...
                    for i in range(1, ni+1):
                        chr_res_df.loc[m, f'pval_i{i}'] =  get_t_pval(
                            chr_res_df.loc[m, f'pval_i{i}'], idof, log=logp
                        )
                        chr_res_df.loc[m, f'pval_gi{i}'] = get_t_pval(
                            chr_res_df.loc[m, f'pval_gi{i}'], idof, log=logp
                        )
                    # substitute column headers
                    chr_res_df.rename(columns=var_dict, inplace=True)
            print('    * writing output')
            outfile = f'{prefix}.cis_qtl_pairs.{chrom}.parquet'
            chr_res_df.to_parquet(os.path.join(output_dir, outfile))
...
def _process_group_permutations(buf, variant_df, start_pos, end_pos, dof,
                                group_id, nperm=10000, beta_approx=True, logp=False):
    """
...
def map_cis(genotype_df, variant_df, phenotype_df, phenotype_pos_df,
    ...
    if covariates_df is not None:
        ...
        assert ~(covariates_df.isnull().any().any()), \
            f'Missing or null values in covariates matrix, in columns {",".join(covariates_df.columns[covariates_df.isnull().any(axis=0)].astype(str))}'
        logger.write(f'  * {covariates_df.shape[1]} covariates')
        residualizer = Residualizer(
            torch.tensor(covariates_df.values, dtype=torch.float32).to(device)
        )
    ...
    permutation_ix_t = torch.LongTensor(np.array([np.random.permutation(ix)
                                                  for i in range(nperm)])).to(device)
    ...
    if group_s is None:
        for k, (phenotype, genotypes, g_idx, haplotypes, _, phenotype_id) in \
                enumerate(igc.generate_data(verbose=verbose), 1):
            # to GPU
            genotypes_t = torch.tensor(genotypes, dtype=torch.float).to(device)
            genotypes_t = genotypes_t[:,genotype_ix_t]
            impute_mean(genotypes_t)
            ...
            var_ix = g_idx[var_ix]
            variant_id = variant_df.index[var_ix]
            start_distance = variant_df['pos'].values[var_ix] - \
                igc.phenotype_start[phenotype_id]
            end_distance = variant_df['pos'].values[var_ix] - \
                igc.phenotype_end[phenotype_id]
            res_s = prepare_cis_output(
                r_nominal, r2_perm, std_ratio, g, genotypes_t.shape[0], idof, variant_id,
                start_distance, end_distance, phenotype_id, nperm=nperm, logp=logp
            )
            if beta_approx:
                res_s[['pval_beta', 'beta_shape1', 'beta_shape2', 'true_df', 'pval_true_df']] = \
        ...
        for k, (phenotypes, genotypes, g_idx, haplotypes, _, phenotype_ids, group_id) in \
                enumerate(igc.generate_data(verbose=verbose), 1):
            # to GPU
            genotypes_t = torch.tensor(genotypes, dtype=torch.float).to(device)
            genotypes_t = genotypes_t[:,genotype_ix_t]
            ...
            if maf_threshold > 0:
                ...
                mask_t = maf_t >= maf_threshold
                genotypes_t = genotypes_t[mask_t]
                haplotypes_t = haplotypes_t[mask_t] if haplotypes_t is not None else None
                mask = mask_t.cpu().numpy().astype(bool)
                g_idx = g_idx[mask]
            ...
            while True:
                ...
                if x[0] <= signif_threshold:
                    ...
                    end_distance = variant_df['pos'].values[var_ix] - \
                        igc.phenotype_end[phenotype_id]
                    res_s = prepare_cis_output(
                        r_nominal, r2_perm, std_ratio, g, genotypes.shape[0], dof, variant_id,
                        start_distance, end_distance, phenotype_id, nperm=nperm, logp=logp
                    )
